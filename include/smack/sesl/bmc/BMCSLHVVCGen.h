...
public:
    ...
    // z3::expr mk_record(std::string lt, z3::expr_vector args);
...
private:
    ...
    std::set<std::string> globalPtrVars;
...
public:
    BlockSemantic(Z3ExprManagerPtr z3EM, RefBlockVertexPtr bptr, RefBlockCFGPtr bcfg, std::set<std::string> global_fable_vars, std::set<std::string> global_ptr_vars)
        : z3EM(z3EM),
        inputs(), outputs(),
        localVars(), quantifiedVars(),
        src(-1), dests(), 
        globalFableVars(global_fable_vars), globalPtrVars(global_ptr_vars),
...
public:
    BMCSLHVVCGen(RefBlockCFGPtr bcfg, RecordManagerPtr rm)
        ...
        // for (auto record : rm->getRecordMap())
        //   z3EM->addRecord(record.first, record.second);
        z3EM->print(std::cout);
        TrSystem = std::make_shared<TREncoder>(z3EM, bcfg);
    }
