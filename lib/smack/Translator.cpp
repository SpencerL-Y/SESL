...
namespace smack{
    ...
    void TransToZ3::translatePure() {
        ...
        auto exp = pExprPtr->translateToZ3(z3_ctx, this->z3VarMap);
    ...
    void TransToZ3::translateSpatial() {
        ...
        for(const SpatialLiteral* sp : spatialList){
            z3SpatialAtoms.push_back(
                sp->translateToZ3(z3_ctx, this->z3VarMap)
            );
        }
    ...
    z3::expr TransToZ3::getZ3Var(
        std::string name, 
        std::unordered_map<std::string, z3::expr>& z3VarMap, z3::context& ctx
    ){
