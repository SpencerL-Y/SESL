                ...
                for p, f, fl in os.walk(path + foldername):
                    for fname in fl:
                        if fname.endswith(".yml"):
                            command1 = 'cp ' + p + "/" + "".join(fname.split(".")[:-1]) + ".c" + " " + newFolderName
    ...
    for path, folder, filename in os.walk(sys.argv[1]):
        for foldername in folder:
            ...
            if foldername == 'termination-crafted':
                for p, f, fl in os.walk(path + foldername):
                    for fname in fl:
                        if fname.endswith(".yml") and (re.match("CWE476*_int*.yml", fname) or re.match("CWE476*_long*.yml", fname) or re.match("CWE476*_struct*.yml", fname) or re.match("CWE121*rand*.yml", fname) or re.match("CWE121*fgets*.yml", fname) or re.match("CWE121*large*.yml", fname) or re.match("CWE401*__int*.yml", fname) or re.match("CWE401*__struct*.yml", fname) or re.match("CWE401*__twoIntsStruct*.yml", fname) or fname.startswith("CWE590")):
                            command1 = 'cp ' + p + "/" + "".join(fname.split(".")[:-1]) + ".c" + " " + newFolderName
                            command2 = 'cp ' + p + "/" + "".join(fname.split(".")[:-1]) + ".yml" + " " + newFolderName
                            os.system(command1)
                            os.system(command2)
