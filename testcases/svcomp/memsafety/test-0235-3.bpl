// generated by SMACK version 2.6.3 for corral
// via /usr/local/bin/smack svcomp/memsafety/test-0235-3.c -ll svcomp/memsafety/test-0235-3_IR.ll --bpl svcomp/memsafety/test-0235-3.bpl -t --sh-mem-leak

const {:global_variable} {:pointer_to_size 16} .str: ref;
axiom (.str == $sub.ref(0, 1026));
const {:global_variable} {:pointer_to_size 248} {:count 31} .str.1: ref;
axiom (.str.1 == $sub.ref(0, 2081));
const {:global_variable} {:pointer_to_size 152} {:count 19} __PRETTY_FUNCTION__.reach_error: ref;
axiom (__PRETTY_FUNCTION__.reach_error == $sub.ref(0, 3124));
const {:global_variable} {:pointer_to_size 64} main.list: ref;
axiom (main.list == $sub.ref(0, 4156));
const reach_error: ref;
axiom (reach_error == $sub.ref(0, 5188));
procedure  reach_error()
{
$bb0:
  call __assert_fail(.str, .str.1, 3, __PRETTY_FUNCTION__.reach_error);
  assume false;
}
const __assert_fail: ref;
axiom (__assert_fail == $sub.ref(0, 6220));
procedure  __assert_fail($p0: ref, $p1: ref, $i2: i32, $p3: ref);
const main: ref;
axiom (main == $sub.ref(0, 7252));
procedure {:entrypoint} main()
  returns ($r: i32)
{
  var $i0: i32;
  var $i1: i1;
  var $p2: ref;
  var $p3: ref;
  var $p4: ref;
  var $p5: ref;
  var $p6: ref;
  var $p7: ref;
  var $p8: ref;
  var $p9: ref;
  var $i10: i1;
  var $i11: i1;
  var $i12: i1;
  var $i13: i32;
$bb0:
  call {:cexpr "smack:entry:main"} boogie_si_record_ref(main);
  goto $bb1;
$bb1:
  call l0_insert(main.list);
  goto $bb2;
$bb2:
  call $i0 := __VERIFIER_nondet_int();
  call {:cexpr "smack:ext:__VERIFIER_nondet_int"} boogie_si_record_i32($i0);
  $i1 := $ne.i32($i0, 0);
  assume {:branchcond $i1} true;
  goto $bb3, $bb4;
$bb3:
  assume ($i1 == 1);
  goto $bb1;
$bb4:
  assume !(($i1 == 1));
  $p2 := $load.ref($M.0, main.list);
  call l0_destroy($p2, 5);
  $p3 := $load.ref($M.0, main.list);
  call l0_destroy($p3, 4);
  $p4 := $load.ref($M.0, main.list);
  call l0_destroy($p4, 3);
  $p5 := $load.ref($M.0, main.list);
  call l0_destroy($p5, 2);
  $p6 := $load.ref($M.0, main.list);
  call l0_destroy($p6, 2);
  $p7 := $load.ref($M.0, main.list);
  call l0_destroy($p7, 1);
  $p8 := $load.ref($M.0, main.list);
  call l0_destroy($p8, 0);
  $p9 := $load.ref($M.0, main.list);
  $i10 := $ne.ref($p9, $0.ref);
  $i11 := $xor.i1($i10, 1);
  $i12 := $xor.i1($i11, 1);
  $i13 := $zext.i1.i32($i12);
  $r := $i13;
  return;
}
const l0_insert: ref;
axiom (l0_insert == $sub.ref(0, 8284));
procedure  l0_insert($p0: ref)
{
  var $p1: ref8;
  var $p2: ref;
  var $p3: ref;
  var $i4: i32;
  var $i5: i1;
  var $p6: ref;
  var $p7: ref;
$bb0:
  call $p1 := zalloc_or_die(16);
  $p2 := $bitcast.ref.ref($p1);
  goto $bb1;
$bb1:
  $p3 := $add.ref($add.ref($p2, $mul.ref(0, 16)), $mul.ref(8, 1));
  call l1_insert($p3);
  goto $bb2;
$bb2:
  call $i4 := __VERIFIER_nondet_int();
  call {:cexpr "smack:ext:__VERIFIER_nondet_int"} boogie_si_record_i32($i4);
  $i5 := $ne.i32($i4, 0);
  assume {:branchcond $i5} true;
  goto $bb3, $bb4;
$bb3:
  assume ($i5 == 1);
  goto $bb1;
$bb4:
  assume !(($i5 == 1));
  $p6 := $load.ref($M.0, $p0);
  $p7 := $add.ref($add.ref($p2, $mul.ref(0, 16)), $mul.ref(0, 1));
  $M.1 := $store.ref($M.1, $p7, $p6);
  $M.0 := $store.ref($M.0, $p0, $p1);
  return;
}
const __VERIFIER_nondet_int: ref;
axiom (__VERIFIER_nondet_int == $sub.ref(0, 9316));
procedure  __VERIFIER_nondet_int()
  returns ($r: i32);
const l0_destroy: ref;
axiom (l0_destroy == $sub.ref(0, 10348));
procedure  l0_destroy($p0: ref, $i1: i32)
{
  var $p2: ref;
  var $i3: i1;
  var $p4: ref;
  var $p5: ref;
  var $p6: ref;
  var $p7: ref;
  var $i8: i1;
  var $p9: ref8;
  var $i10: i1;
$bb0:
  call {:cexpr "l0_destroy:arg:level"} boogie_si_record_i32($i1);
  $p2 := $p0;
  goto $bb1;
$bb1:
  $i3 := $slt.i32(0, $i1);
  assume {:branchcond $i3} true;
  goto $bb2, $bb3;
$bb2:
  assume ($i3 == 1);
  $p4 := $add.ref($add.ref($p2, $mul.ref(0, 16)), $mul.ref(8, 1));
  $p5 := $load.ref($M.1, $p4);
  call l1_destroy($p5, $i1);
  goto $bb4;
$bb3:
  assume !(($i3 == 1));
  goto $bb4;
$bb4:
  $p6 := $add.ref($add.ref($p2, $mul.ref(0, 16)), $mul.ref(0, 1));
  $p7 := $load.ref($M.1, $p6);
  $i8 := $eq.i32(0, $i1);
  assume {:branchcond $i8} true;
  goto $bb5, $bb6;
$bb5:
  assume ($i8 == 1);
  $p9 := $bitcast.ref.ref($p2);
  call free_($p9);
  goto $bb7;
$bb6:
  assume !(($i8 == 1));
  goto $bb7;
$bb7:
  goto $bb8;
$bb8:
  $i10 := $ne.ref($p7, $0.ref);
  assume {:branchcond $i10} true;
  goto $bb9, $bb10;
$bb9:
  assume ($i10 == 1);
  $p2 := $p7;
  goto $bb1;
$bb10:
  assume !(($i10 == 1));
  return;
}
const llvm.dbg.declare: ref;
axiom (llvm.dbg.declare == $sub.ref(0, 11380));
procedure  llvm.dbg.declare($p0: ref, $p1: ref, $p2: ref);
const l1_destroy: ref;
axiom (l1_destroy == $sub.ref(0, 12412));
procedure  l1_destroy($p0: ref, $i1: i32)
{
  var $p2: ref;
  var $i3: i1;
  var $p4: ref;
  var $p5: ref;
  var $p6: ref;
  var $p7: ref;
  var $i8: i1;
  var $p9: ref8;
  var $i10: i1;
$bb0:
  call {:cexpr "l1_destroy:arg:level"} boogie_si_record_i32($i1);
  $p2 := $p0;
  goto $bb1;
$bb1:
  $i3 := $slt.i32(1, $i1);
  assume {:branchcond $i3} true;
  goto $bb2, $bb3;
$bb2:
  assume ($i3 == 1);
  $p4 := $add.ref($add.ref($p2, $mul.ref(0, 16)), $mul.ref(0, 1));
  $p5 := $load.ref($M.1, $p4);
  call l2_destroy($p5, $i1);
  goto $bb4;
$bb3:
  assume !(($i3 == 1));
  goto $bb4;
$bb4:
  $p6 := $add.ref($add.ref($p2, $mul.ref(0, 16)), $mul.ref(8, 1));
  $p7 := $load.ref($M.1, $p6);
  $i8 := $eq.i32(1, $i1);
  assume {:branchcond $i8} true;
  goto $bb5, $bb6;
$bb5:
  assume ($i8 == 1);
  $p9 := $bitcast.ref.ref($p2);
  call free_($p9);
  goto $bb7;
$bb6:
  assume !(($i8 == 1));
  goto $bb7;
$bb7:
  goto $bb8;
$bb8:
  $i10 := $ne.ref($p7, $0.ref);
  assume {:branchcond $i10} true;
  goto $bb9, $bb10;
$bb9:
  assume ($i10 == 1);
  $p2 := $p7;
  goto $bb1;
$bb10:
  assume !(($i10 == 1));
  return;
}
const free_: ref;
axiom (free_ == $sub.ref(0, 13444));
procedure  free_($p0: ref)
{
  call $free($p0);
}
const l2_destroy: ref;
axiom (l2_destroy == $sub.ref(0, 14476));
procedure  l2_destroy($p0: ref, $i1: i32)
{
  var $p2: ref;
  var $i3: i1;
  var $p4: ref;
  var $p5: ref;
  var $p6: ref;
  var $p7: ref;
  var $i8: i1;
  var $p9: ref8;
  var $i10: i1;
$bb0:
  call {:cexpr "l2_destroy:arg:level"} boogie_si_record_i32($i1);
  $p2 := $p0;
  goto $bb1;
$bb1:
  $i3 := $slt.i32(2, $i1);
  assume {:branchcond $i3} true;
  goto $bb2, $bb3;
$bb2:
  assume ($i3 == 1);
  $p4 := $add.ref($add.ref($p2, $mul.ref(0, 16)), $mul.ref(8, 1));
  $p5 := $load.ref($M.1, $p4);
  call l3_destroy($p5, $i1);
  goto $bb4;
$bb3:
  assume !(($i3 == 1));
  goto $bb4;
$bb4:
  $p6 := $add.ref($add.ref($p2, $mul.ref(0, 16)), $mul.ref(0, 1));
  $p7 := $load.ref($M.1, $p6);
  $i8 := $eq.i32(2, $i1);
  assume {:branchcond $i8} true;
  goto $bb5, $bb6;
$bb5:
  assume ($i8 == 1);
  $p9 := $bitcast.ref.ref($p2);
  call free_($p9);
  goto $bb7;
$bb6:
  assume !(($i8 == 1));
  goto $bb7;
$bb7:
  goto $bb8;
$bb8:
  $i10 := $ne.ref($p7, $0.ref);
  assume {:branchcond $i10} true;
  goto $bb9, $bb10;
$bb9:
  assume ($i10 == 1);
  $p2 := $p7;
  goto $bb1;
$bb10:
  assume !(($i10 == 1));
  return;
}
const l3_destroy: ref;
axiom (l3_destroy == $sub.ref(0, 15508));
procedure  l3_destroy($p0: ref, $i1: i32)
{
  var $p2: ref;
  var $i3: i1;
  var $p4: ref;
  var $p5: ref;
  var $p6: ref;
  var $p7: ref;
  var $i8: i1;
  var $p9: ref8;
  var $i10: i1;
$bb0:
  call {:cexpr "l3_destroy:arg:level"} boogie_si_record_i32($i1);
  $p2 := $p0;
  goto $bb1;
$bb1:
  $i3 := $slt.i32(3, $i1);
  assume {:branchcond $i3} true;
  goto $bb2, $bb3;
$bb2:
  assume ($i3 == 1);
  $p4 := $add.ref($add.ref($p2, $mul.ref(0, 16)), $mul.ref(0, 1));
  $p5 := $load.ref($M.1, $p4);
  call l4_destroy($p5, $i1);
  goto $bb4;
$bb3:
  assume !(($i3 == 1));
  goto $bb4;
$bb4:
  $p6 := $add.ref($add.ref($p2, $mul.ref(0, 16)), $mul.ref(8, 1));
  $p7 := $load.ref($M.1, $p6);
  $i8 := $eq.i32(3, $i1);
  assume {:branchcond $i8} true;
  goto $bb5, $bb6;
$bb5:
  assume ($i8 == 1);
  $p9 := $bitcast.ref.ref($p2);
  call free_($p9);
  goto $bb7;
$bb6:
  assume !(($i8 == 1));
  goto $bb7;
$bb7:
  goto $bb8;
$bb8:
  $i10 := $ne.ref($p7, $0.ref);
  assume {:branchcond $i10} true;
  goto $bb9, $bb10;
$bb9:
  assume ($i10 == 1);
  $p2 := $p7;
  goto $bb1;
$bb10:
  assume !(($i10 == 1));
  return;
}
const l4_destroy: ref;
axiom (l4_destroy == $sub.ref(0, 16540));
procedure  l4_destroy($p0: ref, $i1: i32)
{
  var $p2: ref;
  var $i3: i1;
  var $p4: ref;
  var $p5: ref;
  var $p6: ref8;
  var $p7: ref;
  var $p8: ref;
  var $i9: i1;
  var $p10: ref8;
  var $i11: i1;
$bb0:
  call {:cexpr "l4_destroy:arg:level"} boogie_si_record_i32($i1);
  $p2 := $p0;
  goto $bb1;
$bb1:
  $i3 := $eq.i32(5, $i1);
  assume {:branchcond $i3} true;
  goto $bb2, $bb3;
$bb2:
  assume ($i3 == 1);
  $p4 := $add.ref($add.ref($p2, $mul.ref(0, 16)), $mul.ref(8, 1));
  $p5 := $load.ref($M.1, $p4);
  $p6 := $bitcast.ref.ref($p5);
  call free_($p6);
  goto $bb4;
$bb3:
  assume !(($i3 == 1));
  goto $bb4;
$bb4:
  $p7 := $add.ref($add.ref($p2, $mul.ref(0, 16)), $mul.ref(0, 1));
  $p8 := $load.ref($M.1, $p7);
  $i9 := $eq.i32(4, $i1);
  assume {:branchcond $i9} true;
  goto $bb5, $bb6;
$bb5:
  assume ($i9 == 1);
  $p10 := $bitcast.ref.ref($p2);
  call free_($p10);
  goto $bb7;
$bb6:
  assume !(($i9 == 1));
  goto $bb7;
$bb7:
  goto $bb8;
$bb8:
  $i11 := $ne.ref($p8, $0.ref);
  assume {:branchcond $i11} true;
  goto $bb9, $bb10;
$bb9:
  assume ($i11 == 1);
  $p2 := $p8;
  goto $bb1;
$bb10:
  assume !(($i11 == 1));
  return;
}
const zalloc_or_die: ref;
axiom (zalloc_or_die == $sub.ref(0, 17572));
procedure  zalloc_or_die($i0: i32)
  returns ($r: ref)
{
  var $i1: i64;
  var $p2: ref8;
  var $i3: i1;
$bb0:
  call {:cexpr "zalloc_or_die:arg:size"} boogie_si_record_i32($i0);
  $i1 := $zext.i32.i64($i0);
  call $p2 := calloc_model(1, $i1);
  $i3 := $ne.ref($p2, $0.ref);
  assume {:branchcond $i3} true;
  goto $bb1, $bb2;
$bb1:
  assume ($i3 == 1);
  $r := $p2;
  return;
$bb2:
  assume !(($i3 == 1));
  call abort();
  assume false;
}
const l1_insert: ref;
axiom (l1_insert == $sub.ref(0, 18604));
procedure  l1_insert($p0: ref)
{
  var $p1: ref8;
  var $p2: ref;
  var $p3: ref;
  var $i4: i32;
  var $i5: i1;
  var $p6: ref;
  var $p7: ref;
$bb0:
  call $p1 := zalloc_or_die(16);
  $p2 := $bitcast.ref.ref($p1);
  goto $bb1;
$bb1:
  $p3 := $add.ref($add.ref($p2, $mul.ref(0, 16)), $mul.ref(0, 1));
  call l2_insert($p3);
  goto $bb2;
$bb2:
  call $i4 := __VERIFIER_nondet_int();
  call {:cexpr "smack:ext:__VERIFIER_nondet_int"} boogie_si_record_i32($i4);
  $i5 := $ne.i32($i4, 0);
  assume {:branchcond $i5} true;
  goto $bb3, $bb4;
$bb3:
  assume ($i5 == 1);
  goto $bb1;
$bb4:
  assume !(($i5 == 1));
  $p6 := $load.ref($M.1, $p0);
  $p7 := $add.ref($add.ref($p2, $mul.ref(0, 16)), $mul.ref(8, 1));
  $M.1 := $store.ref($M.1, $p7, $p6);
  $M.1 := $store.ref($M.1, $p0, $p1);
  return;
}
const l2_insert: ref;
axiom (l2_insert == $sub.ref(0, 19636));
procedure  l2_insert($p0: ref)
{
  var $p1: ref8;
  var $p2: ref;
  var $p3: ref;
  var $i4: i32;
  var $i5: i1;
  var $p6: ref;
  var $p7: ref;
$bb0:
  call $p1 := zalloc_or_die(16);
  $p2 := $bitcast.ref.ref($p1);
  goto $bb1;
$bb1:
  $p3 := $add.ref($add.ref($p2, $mul.ref(0, 16)), $mul.ref(8, 1));
  call l3_insert($p3);
  goto $bb2;
$bb2:
  call $i4 := __VERIFIER_nondet_int();
  call {:cexpr "smack:ext:__VERIFIER_nondet_int"} boogie_si_record_i32($i4);
  $i5 := $ne.i32($i4, 0);
  assume {:branchcond $i5} true;
  goto $bb3, $bb4;
$bb3:
  assume ($i5 == 1);
  goto $bb1;
$bb4:
  assume !(($i5 == 1));
  $p6 := $load.ref($M.1, $p0);
  $p7 := $add.ref($add.ref($p2, $mul.ref(0, 16)), $mul.ref(0, 1));
  $M.1 := $store.ref($M.1, $p7, $p6);
  $M.1 := $store.ref($M.1, $p0, $p1);
  return;
}
const l3_insert: ref;
axiom (l3_insert == $sub.ref(0, 20668));
procedure  l3_insert($p0: ref)
{
  var $p1: ref8;
  var $p2: ref;
  var $p3: ref;
  var $i4: i32;
  var $i5: i1;
  var $p6: ref;
  var $p7: ref;
$bb0:
  call $p1 := zalloc_or_die(16);
  $p2 := $bitcast.ref.ref($p1);
  goto $bb1;
$bb1:
  $p3 := $add.ref($add.ref($p2, $mul.ref(0, 16)), $mul.ref(0, 1));
  call l4_insert($p3);
  goto $bb2;
$bb2:
  call $i4 := __VERIFIER_nondet_int();
  call {:cexpr "smack:ext:__VERIFIER_nondet_int"} boogie_si_record_i32($i4);
  $i5 := $ne.i32($i4, 0);
  assume {:branchcond $i5} true;
  goto $bb3, $bb4;
$bb3:
  assume ($i5 == 1);
  goto $bb1;
$bb4:
  assume !(($i5 == 1));
  $p6 := $load.ref($M.1, $p0);
  $p7 := $add.ref($add.ref($p2, $mul.ref(0, 16)), $mul.ref(8, 1));
  $M.1 := $store.ref($M.1, $p7, $p6);
  $M.1 := $store.ref($M.1, $p0, $p1);
  return;
}
const l4_insert: ref;
axiom (l4_insert == $sub.ref(0, 21700));
procedure  l4_insert($p0: ref)
{
  var $p1: ref8;
  var $p2: ref;
  var $p3: ref8;
  var $p4: ref;
  var $p5: ref;
  var $p6: ref;
  var $p7: ref;
$bb0:
  call $p1 := zalloc_or_die(16);
  $p2 := $bitcast.ref.ref($p1);
  call $p3 := zalloc_or_die(119);
  $p4 := $bitcast.ref.ref($p3);
  $p5 := $add.ref($add.ref($p2, $mul.ref(0, 16)), $mul.ref(8, 1));
  $M.1 := $store.ref($M.1, $p5, $p3);
  $p6 := $load.ref($M.1, $p0);
  $p7 := $add.ref($add.ref($p2, $mul.ref(0, 16)), $mul.ref(0, 1));
  $M.1 := $store.ref($M.1, $p7, $p6);
  $M.1 := $store.ref($M.1, $p0, $p1);
  return;
}
const calloc_model: ref;
axiom (calloc_model == $sub.ref(0, 22732));
procedure  calloc_model($i0: i64, $i1: i64)
  returns ($r: ref)
{
  var $i2: i64;
  var $p3: ref8;
  var $i4: i64;
$bb0:
  call {:cexpr "calloc_model:arg:nmemb"} boogie_si_record_i64($i0);
  call {:cexpr "calloc_model:arg:size"} boogie_si_record_i64($i1);
  $i2 := $mul.i64($i0, $i1);
  call $p3 := malloc($i2);
  $i4 := $mul.i64($i0, $i1);
  call $M.1 := $memset.i8($M.1, $p3, 0, $i4, false);
  $r := $p3;
  return;
}
const abort: ref;
axiom (abort == $sub.ref(0, 23764));
procedure  abort();
const malloc: ref;
axiom (malloc == $sub.ref(0, 24796));
procedure  malloc($i0: i64)
  returns ($r: ref)
{
  call $r := $malloc($i0);
}
const llvm.memset.p0i8.i64: ref;
axiom (llvm.memset.p0i8.i64 == $sub.ref(0, 25828));
procedure  llvm.memset.p0i8.i64($p0: ref, $i1: i8, $i2: i64, $i3: i1);
const llvm.dbg.value: ref;
axiom (llvm.dbg.value == $sub.ref(0, 26860));
procedure  llvm.dbg.value($p0: ref, $p1: ref, $p2: ref);
const __SMACK_static_init: ref;
axiom (__SMACK_static_init == $sub.ref(0, 27892));
procedure  __SMACK_static_init()
{
$bb0:
  $M.0 := $store.ref($M.0, main.list, $0.ref);
  return;
}
procedure  $memset.i8(M: [ref] i8, dst: ref, val: i8, len: ref, isvolatile: bool) returns (M.ret: [ref] i8)
{
  assume (forall x: ref :: $sle.ref.bool(dst,x) && $slt.ref.bool(x,$add.ref(dst,len)) ==> M.ret[x] == val);
  assume (forall x: ref :: $slt.ref.bool(x,dst) ==> M.ret[x] == M[x]);
  assume (forall x: ref :: $sle.ref.bool($add.ref(dst,len),x) ==> M.ret[x] == M[x]);
}

procedure  boogie_si_record_i32(x: i32);
procedure  boogie_si_record_i64(x: i64);
procedure  boogie_si_record_ref(x: ref);
procedure  $initialize()
{
  call __SMACK_static_init();
  return;
}
