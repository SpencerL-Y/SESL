...
class TestRunner():
    ...
    def prework4TestCases(self, fileNames):
        ...
        for file_name in fileNames:
            ...
            #     continue
            ftype = file_info[-1]
            file_name = file_name[:-len(ftype)-1]
            if ftype not in {'c', 'i', self.propertyType}:
                continue
            files.add((file_name, ftype))
            if ftype == 'c' or ftype == 'i':
                c_file[file_name] = True
            elif ftype == self.propertyType:
        ...
        for name, ftype in files:
            ...
            if ftype == self.propertyType: continue
            self.testCases.append((name, ftype, program_property))
    ...
    def run(self):
        for name, ftype, prop in self.testCases:
            try:
                self.run_single_file(name, ftype, prop)
            except MemoryError:
...
if __name__ == "__main__":
    ...
    for path, folders, _ in os.walk(in_path):
        ...
        print("\nAnalysising benchmark {}:".format(info[-1]))
